...
from vertex_choose import closest_neighbor_vertex_choose as vertex_choose_func
from original_simulated_anneling import simulated_annealing as simulated_annealing_func
from multiple_simulated_anneling import multiple_runs_simulated_annealing as multiple_simulated_annealing_func
from ant_colony import ant_colony_func
...
def original_simulated_annealing_wrapper(file_path):
    start_time = time.time()
    _, _, total_weight = simulated_annealing_func(file_path)
    end_time = time.time()
    time_spent = end_time - start_time
    return time_spent, total_weight

def multiple_annealing_wrapper(file_path):
    time_spent, _, total_weight = multiple_simulated_annealing_func(file_path)
    return time_spent, total_weight

def ant_colony_wrapper(file_path):
    start_time = time.time()
    _, _, total_weight = ant_colony_func(file_path)
    end_time = time.time()
    time_spent = end_time - start_time
    return time_spent, total_weight

def mmas_wrapper(file_path):
    start_time = time.time()
    _, _, total_weight = mmas(file_path)
    end_time = time.time()
    time_spent = end_time - start_time
    return time_spent, total_weight
...
def create_gui():
    ...
    def execute_functions(file_paths):
        num_vertices_list = []
        times_list = [[] for _ in range(7)]
        total_weights_list = [[] for _ in range(7)]
        ...
        for file_path in file_paths:
            ...
            for i, func in enumerate([original_func_wrapper, heap_store_func_wrapper, vertex_choose_func_wrapper,original_simulated_annealing_wrapper, multiple_annealing_wrapper,ant_colony_wrapper, mmas_wrapper]):
    ...
    def plot_graph(x_data, y_data, ylabel, title):
        plt.figure()
        function_names = ["Closest Neighbour(CN)", "Heap Vertex(CN)", "Vertex Choose(CN)","Simulated Annealing(SA)","Muliple Anneling(SA)","Ant Colony(AC)","MMAS(AC)"]
